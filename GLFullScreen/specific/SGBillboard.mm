...
void bbSet(sgBillboard *b, mVec3 *e, mVec3 *up, mVec3 *p, float sz, int tx, int ty) {
    ...
    t1y = 1.0-(ty * BB_INV_DENSITY);  // textures can be inverted
...
void bbUpdate(sgBillboard *b, mVec3 *e, mVec3 *up, mVec3 *viewDir, mVec3 *p, float sz) {
    mVec3 cam, vert, horiz, camNorm, vizCross;
    mVec3 c1, c2, c3, c4, spare; 
    double vizCrLen;
    
    mvCopy(&(b->pos), p); 
    
    mvEqSub(&cam, p, e); // cam is eye to sprite pos

    // check visibility-- fuck off if it's invisible
    mvCopy(&camNorm, &cam); // norm of cam
    mvNormalize(&camNorm); 
    mvEqCross(&vizCross, &camNorm, viewDir);
    mvLength3sq(&vizCross, &vizCrLen);
   // NSLog(@"v %f %f %f   cr %f %f %f   vcrl %f", viewDir->x, viewDir->y, viewDir->z, vizCross.x, vizCross.y, vizCross.z, vizCrLen);
    if (vizCrLen < .45) {
        // angle is less than 60 degrees
    
        // compute depth for sort
        mvLength3sq(&cam, &(b->depthSq));
